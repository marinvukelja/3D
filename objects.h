...
public
  ...
  bool ray_intersect(const Ray &ray, float &t, Vec3f &normal) const {
    float best_t = stdnumeric_limitsfloatmax();
    bool result;
...

    {
      ...
      if (stdmin(a1.x, b1.x) = p.x && p.x = stdmax(a1.x, b1.x) &&
          stdmin(a1.y, b1.y) = p.y && p.y = stdmax(a1.y, b1.y)) {
        if (da1  best_t) {
          result = true;
          best_t = da1;
          t = da1;
...

    {
      ...
      if (stdmin(b1.y, c1.y) = p.y && p.y = stdmax(b1.y, c1.y) &&
          stdmin(b1.z, c1.z) = p.z && p.z = stdmax(b1.z, c1.z)) {
        if (db1  best_t) {
          result = true;
          best_t = db1;
          t = db1;
...

    {
      ...
      if (stdmin(c1.x, a1.x) = p.x && p.x = stdmax(c1.x, a1.x) &&
          stdmin(c1.z, a1.z) = p.z && p.z = stdmax(c1.z, a1.z)) {
        if (dc1  best_t) {
          result = true;
          best_t = dc1;
          t = dc1;
...

    {
      ...
      if (stdmin(a2.x, b2.x) = p.x && p.x = stdmax(a2.x, b2.x) &&
          stdmin(a2.y, b2.y) = p.y && p.y = stdmax(a2.y, b2.y)) {
        if (da2  best_t) {
          result = true;
          best_t = da2;
          t = da2;
...

    {
      ...
      if (stdmin(b2.y, c2.y) = p.y && p.y = stdmax(b2.y, c2.y) &&
          stdmin(b2.z, c2.z) = p.z && p.z = stdmax(b2.z, c2.z)) {
        if (db2  best_t) {
          result = true;
          best_t = db2;
          t = db2;
...

    {
      ...
      if (stdmin(c2.x, a2.x) = p.x && p.x = stdmax(c2.x, a2.x) &&
          stdmin(c2.z, a2.z) = p.z && p.z = stdmax(c2.z, a2.z)) {
        if (dc2  best_t) {
          result = true;
          best_t = dc2;
          t = dc2;
